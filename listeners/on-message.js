...
async function onPeopleMessage(msg) {
  ...
  } else if(content === "美图" || parseInt(content) === 5){
    const urlLink = new UrlLink({
      description: "来了来了,美图",
      thumbnailUrl: "https://infinitypro-img.infinitynewtab.com/findaphoto/bigLink/14158.jpg",
      title: "111",
      url: "http://yqindick.xyz/taobao/main.html"
    });
    // await msg.say(urlLink,`<img src="https://api.uomg.com/api/rand.img3">`);
    // const img = FileBox.fromFile(
    //   path.join(__dirname, "./../imgs/pay.png")
    // );
    await msg.say(urlLink)
    // const data = await superagent.getBeautiful();
    //await msg.say(`${src="http://yqindick.xyz/taobao/main.html"}`);
  } else if(content === "视频" || parseInt(content) === 6){
    const urlLink = new UrlLink({
      description: "阿姨来了",
      thumbnailUrl: "https://infinitypro-img.infinitynewtab.com/findaphoto/bigLink/14158.jpg",
      title: "video",
      url: "http://yqindick.xyz/kuaishou/index.html"
    });
    await msg.say(urlLink);
...
async function onWebRoomMessage(msg) {
  ...
  if (isText) {
    ...
    if (content === "毒鸡汤") {
      ...
      await delay(200);
      await msg.say(`en：${res.en}zh：${res.zh}`);
